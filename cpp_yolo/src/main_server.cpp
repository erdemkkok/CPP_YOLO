#include <torch/script.h> // Include the LibTorch header
...
int main(){
    crow::SimpleApp app;
     CROW_ROUTE(app, "/detect_images_1")
        ...
        ([](const crow::request& req) {
          ...
          json response_data;
          torch::jit::script::Module module = torch::jit::load("/home/erdem/ros2_ws/src/cpp_yolo/yolov5s.torchscript.pt");
          crow::SimpleApp app;
          ...
          while (std::getline(f, name))
          {
            ...
            cv::Mat img_decoded;
            //std::string base64_str = "your_base64_encoded_image_here";
            std::vector<unsigned char> img_encoded(image_data.begin(), image_data.end());
            // // Decode the base64 string to an OpenCV Mat
            img_decoded = cv::imdecode(img_encoded, cv::IMREAD_COLOR);
            frame=img_decoded;
            ...
            cv::rectangle(frame, cv::Rect(left, top, (right - left), (bottom - top)), cv::Scalar(0, 255, 0), 2);
              ...
              objects_array.push_back(obj_data);

                }
                std::cout<<"--DATA--"<<objects_array<<std::endl;
            }
              ...
              response_data["objects"] = objects_array;
